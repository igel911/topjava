...
public class JdbcUserRepositoryImpl implements UserRepository {
    ...
    public List<User> getAll() {
        return jdbcTemplate.query("SELECT * FROM users LEFT JOIN user_roles " +
                "ON users.id = user_roles.user_id ORDER BY name, email", getResultSetExtractor());
    }
        ...
        return rs -> {
            ...
            while (rs.next()) {
                ...
                if (users.contains(user)) {
                    users.get(users.indexOf(user)).getRoles().add(Role.valueOf(sRole));
                } else if (Objects.nonNull(sRole)){
                    user.setRoles(EnumSet.of(Role.valueOf(sRole)));
                    users.add(user);
                }
            }
            return users;
